<template>
  <div class="content">
    <div class="header">
      <div class="total fl">
        <span class="text">共{{data.length}}封邮件</span>
      </div>
      <el-button class="MarkReaded fl" @click="handleMarkReaded" v-if='type === "in"' size="small">全部标记为已读</el-button>
      <div class="page fr">
    ...
    <table>
      ...
      <tbody>
        <tr
          ...
          <th>{{item.date}}</th>
          <th @click.stop="star(item)">
            <i :class='item.isStar ? "el-icon-star-on" : "el-icon-star-off"'></i>
          </th>
        </tr>
      </tbody>
    </table>
  ...
  computed: {
    ...
    currentData () {
      ...
      return this.data.slice((page - 1) * PERPAGE, page * PERPAGE).map(item => ({
        ...
        data: this.$moment(item.date).format('YYYY-MM-DD HH:mm:ss'),
